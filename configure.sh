...
The following flags enable optional features (disable with --no-<option name>).
  ...
  --only-python-ext-src    create only Python bindings source files
...
do
  case $1 in
    ...
    --only-python-ext-src) only_python_ext_src=ON;;
...
[ $only_python_ext_src != default ] \
  && cmake_opts="$cmake_opts -DONLY_PYTHON_EXT_SRC=$only_python_ext_src"
[ $java_bindings != default ] \
