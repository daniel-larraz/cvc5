...
bool BasicRewriteRCons::ensureProofMacroDtConsEq(CDProof* cdp, const Node& eq)
{
  ...
  if (!expr::isACINorm(rhs, eq[1]))
  {
    Trace("brc-macro") << "Failed to show " << rhs << " == " << eq[1]
                       << std::endl;
    AssertFalse() << "Failed to show " << rhs << " == " << eq[1] << std::endl;
    return false;
  }
  // use ACI_NORM
  cdp->addProof(pfn);
                                                       ...
                                                       const Node& eq)
{
  ...
  {
    AssertFalse() << "Unexpected input ensureProofMacroQuantMiniscope " << eq;
    return false;
  }
  ProofChecker* pc = d_env.getProofNodeManager()->getChecker();
  ...
  for (size_t i = 0, nconj = mq.getNumChildren(); i < nconj; i++)
  {
    ...
    AssertFalse() << "Failed ensureProofMacroQuantMiniscope " << eq;
    return false;
  }
                                                         ...
                                                         const Node& eq)
{
  ...
  if (qr != eq[1])
  {
    AssertFalse() << "Failed to rewrite " << eq[0] << " to " << qr
                  << " != " << eq[1];
    return false;
  }
  std::shared_ptr<ProofNode> pfn = tcpg.getProofFor(eq);
