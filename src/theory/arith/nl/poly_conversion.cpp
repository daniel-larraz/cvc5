                                           ...
                                           const cvc5::internal::Node& var)
{
  ...
  switch (n.getKind())
  {
    ...
    default:
      AssertFalse() << "Unhandled node " << n << " with kind " << n.getKind()
                << std::endl;
  }
                                   ...
                                   poly::Polynomial& lhs)
{
  switch (kind)
  {
    ...
    default:
      AssertFalse() << "This function only deals with arithmetic relations.";
...
Node value_to_node(const poly::Value& v, const Node& ran_variable)
{
  ...
  AssertFalse() << "All cases should be covered.";
...
std::tuple<Node, Rational, Rational> detect_ran_encoding(const Node& w)
{
  ...
  if (n[0].getKind() == Kind::EQUAL)
    ...
    AssertFalse() << "Could not identify polynomial equation.";
  ...
  {
    AssertFalse() << "Invalid polynomial equation.";
  }
