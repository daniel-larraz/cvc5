  ...
  switch (infer)
  {
    ...
    case InferenceId::STRINGS_SSPLIT_VAR:
    {
      ...
      else
      {
        ...
        else
        {
          ...
          AssertFalse() << "No reconstruction rule given for " << infer;
        }
      }
    }
    break;
    ...
    case InferenceId::STRINGS_DEQ_DISL_STRINGS_SPLIT:
    {
          ...
          || conc[1][0].getKind() != Kind::STRING_LENGTH)
      {
        ...
        AssertFalse() << "unexpected conclusion " << conc << " for " << infer;
      }
      else
      {
    ...
    case InferenceId::STRINGS_UNIT_SPLIT:
    {
      if (conc.getKind() != Kind::OR)
      {
        ...
        AssertFalse() << "Expected OR conclusion for " << infer;
      }
      else
      {
    ...
    case InferenceId::STRINGS_RE_UNFOLD_NEG:
    {
      ...
      if (mem.isNull())
      {
        ...
        AssertFalse() << "Failed to apply MACRO_SR_PRED_ELIM for RE unfold";
        useBuffer = false;
      }
    ...
    case InferenceId::STRINGS_REDUCTION:
    {
      ...
      if (mainEq.isNull())
      {
        ...
        AssertFalse() << "Unexpected reduction " << conc;
        break;
      }
    ...
    case InferenceId::STRINGS_PREFIX_CONFLICT_MIN:
    {
      ...
      for (const Node& e : ps.d_children)
      {
        ...
        else
        {
          ...
          AssertFalse() << "Unexpected premise " << e << " for " << infer;
        }
      }
      if (eqs.empty())
      {
  ...
  for (size_t i = 0; i < n; i++)
  {
             ...
             && conc.getKind() == Kind::EQUAL)
    {
      ...
      for (size_t j = 0; j < 2; j++)
      {
        ...
        {
          AssertFalse() << "Non-constant for unify";
          return eq;
        }
        ...
        if (len <= lentgt)
        {
          AssertFalse() << "Smaller source for unify";
          return eq;
        }
        if (isRev)
        {
    ...
    else if (rule == ProofRule::CONCAT_CSPLIT)
    {
      if (!currS.isConst())
      {
        AssertFalse() << "Non-constant for csplit";
        return eq;
      }
             ...
             && !conc.getConst<bool>())
    {
      if (!currT.isConst() || !currS.isConst())
      {
        AssertFalse() << "Non-constants for concat conflict";
        return eq;
      }
    ...
    {
      AssertFalse() << "Unknown rule to splice " << rule;
      return eq;
    }
    ...
    {
      AssertFalse() << "Failed to show " << eqr << " spliced from " << eq;
      return eq;
    }
    return eqr;
  }
