...
Node SequencesRewriter::rewriteViaOverlap(ProofRewriteRule id, const Node& n)
{
  ...
  switch (id)
  {
    ...
    case ProofRewriteRule::STR_OVERLAP_ENDPOINTS_REPLACE:
      return nm->mkNode(
          Kind::STRING_CONCAT,
          {n[0][0], nm->mkNode(k, {n[0][1], n[1], n[2]}), n[0][2]});
    default: break;
  }
  return Node::null();
}
      ...
      if (node[2].isConst() && node[2].getConst<Rational>().sgn() == 0)
      {
                ...
                children0, children1, nb, ne, 1))
        {
          ...
          Node ret = nm->mkNode(
              ...
              nm->mkNode(
                  Kind::STRING_INDEXOF,
                  {utils::mkConcat(children0, stype), node[1], node[2]}));
...
Node SequencesRewriter::rewriteReplace(Node node)
{
    ...
    if (d_arithEntail.check(maxLen1, len0_1, true))
    {
      children1.push_back(nm->mkNode(
          ...
          {lastChild1[0],
           lastChild1[1],
           nm->mkNode(Kind::ADD,
                      {len0, one, nm->mkNode(Kind::NEG, partLen1)})}));
      Node res =
          nm->mkNode(Kind::STRING_REPLACE,
                     {node[0], utils::mkConcat(children1, stype), node[2]});
    ...
    if (x[0] == x[2] && x[0] == y)
    {
      ...
      if (d_arithEntail.check(wlen, zlen))
      {
        ...
        if (w != z && w.isConst() && z.isConst())
        {
          Node ret =
              nm->mkNode(Kind::STRING_REPLACE,
                         {nm->mkNode(Kind::STRING_REPLACE, {y, w, z}), y, z});
  ...
  if (node[1].getKind() == Kind::STRING_REPLACE)
  {
    ...
    {
      Node res =
          nm->mkNode(Kind::STRING_REPLACE, {node[0], node[1][0], node[2]});
